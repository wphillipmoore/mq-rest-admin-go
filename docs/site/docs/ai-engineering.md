...
`mqrestadmin` is an AI-assisted engineering project. Significant portions of
...
**Mapping data bootstrap**: The `mapping-data.json` embedded resource was
...
**Quality standards**: The requirement for comprehensive test coverage,
race detection (`go test -race`), static analysis (golangci-lint), and
the validation gate pipeline were human-defined standards.

**Design decisions**: Key choices like the mapping opt-out mechanism,
strict vs lenient modes, the transport abstraction, Go-idiomatic error
values (vs exception hierarchies), and `context.Context` threading were
human decisions refined through discussion.
